...
export async function decide(order: OrderInfo, currentBlock: number): Promise<FillDecision> {
  ...
  // ── Market spread check ───────────────────────────────────────────────────
  // Fake market reference (dev): no real price feed, so treat "market" as a small
  // premium above the order's own start price. The frontend sets startPrice from
  // the live market estimate, so this is a per-order proxy that works for ANY pair
  // with no hardcoded price table — the filler always perceives a spread and fills.
  const refPrice = (BigInt(order.startPrice) * BigInt(10_000 + INVENTORY.FAKE_MARKET_PREMIUM_BPS)) / 10_000n
