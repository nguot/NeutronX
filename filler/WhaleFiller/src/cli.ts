...
async function runFill(hash: string, pct: number): Promise<void> {
  ...
  try {
    ...
    spin.succeed(`filled ${shortHash(hash)} @ ${pct}%   ${c.dim('tx')} ${tx}`)
  } catch (e: any) {
...
// ── cross-chain ────────────────────────────────────────────────────────────────
async function renderCcOrders(): Promise<void> {
...
async function runCcOp(label: string, hash: string, slot: number, fn: () => Promise<string>): Promise<void> {
...
async function runQuote(opts: any): Promise<void> {
  const block = await currentBlock()
...
async function withWatch(seconds: number | undefined, render: () => Promise<void>): Promise<void> {
...
program.name('whalefiller').description(`NeutronX ${FILLER_NAME} dev CLI`).version('1.0.0')
...
program.command('sim <hash>').description('dry-run a fill preview (no transaction)')
  .option('-p, --pct <n>', 'percent of remaining to fill', '50')
  .action((hash, o) => renderSim(hash, Number(o.pct)))

program.command('fill <hash>').description('execute a partial fill')
  .option('-p, --pct <n>', 'percent of remaining to fill', '100')
  .action((hash, o) => runFill(hash, Number(o.pct)))
...
cc.command('fill <hash> <slot>').description('fill a cross-chain slot (deploy dst escrow → withdraw on src)')
  .action((hash, slot) => runCcOp('cc fill', hash, Number(slot), () => crossChainFill(hash, Number(slot))))
cc.command('claim <hash> <slot>').description('recover a slot the backend already claimed (withdraw on src)')
  .action((hash, slot) => runCcOp('cc claim', hash, Number(slot), () => crossChainClaim(hash, Number(slot))))
cc.command('reset <hash> <slot>').description('reset a stuck locked slot back to available')
  .action((hash, slot) => runCcOp('cc reset', hash, Number(slot), async () => { await resetSlot(hash, Number(slot)); return 'reset' }))
