#!/usr/bin/env bash
# run_cc_reverse.sh — end-to-end cross-chain swap, B→A direction
# Mirror image of run_cc.sh with roles swapped: Chain B is now the SOURCE
# (swapper locks USDC via Permit2) and Chain A is now the DESTINATION
# (fillers deposit WETH into EscrowDst clones).
# Two fillers concurrently fill two different slots of the same order.
#
# Chain B (port 8546): EscrowSrcFactory (ESCROW_SRC_FACTORY_B) — swapper's USDC
#                       pulled per-slot via Permit2
# Chain A (port 8545): EscrowDstFactory (CHAIN_A_DST_FACTORY) — fillers deploy
#                       isolated escrow clones holding WETH
#
# Chain B filler flow, run independently by EACH filler for its own slot:
...
#      pulls slotAmount of USDC from the swapper via Permit2)
#   2. EscrowSrc(escrow).withdraw(S_i)  — once S_i is public on Chain A
#
# Chain A filler flow (unchanged, NO approve needed):
#   1. computeAddress(H_i, filler) → escrowAddr  (free, off-chain)
#   2. WETH.transfer(escrowAddr, amount)          (tokens land at future clone)
#   3. factory.deploy(H_i, swapper, WETH, ...)   (deploys clone, verifies balance)
#
# Prerequisites:
...
#   setup_cc.sh done: logs/.cc_addresses has ESCROW_SRC_FACTORY_B + CHAIN_A_DST_FACTORY
...
export LOG_FILE="$LOG_DIR/run_cc_reverse_$(date +%Y%m%d_%H%M%S).log"
...
source "$ADDR_FILE"   # ESCROW_SRC_FACTORY_B, CHAIN_A_DST_FACTORY, COSIGNER
[ -z "$ESCROW_SRC_FACTORY_B" ] && { log ERROR "ESCROW_SRC_FACTORY_B not in $ADDR_FILE — re-run setup_cc.sh"; exit 1; }
[ -z "$CHAIN_A_DST_FACTORY" ]  && { log ERROR "CHAIN_A_DST_FACTORY not in $ADDR_FILE — re-run setup_cc.sh"; exit 1; }
log OK "Chain B EscrowSrcFactory  : $ESCROW_SRC_FACTORY_B"
log OK "Chain A EscrowDstFactory  : $CHAIN_A_DST_FACTORY"
log OK "Cosigner                  : $COSIGNER"

# ── 2. Snapshot balances ──────────────────────────────────────────────────────
log STEP "Step 2 — Snapshotting balances (both fillers + swapper)"

declare -a USDC_B_BEFORE WETH_A_BEFORE
for i in 0 1; do
  ...
  log INFO "${FILLER_NAMES[$i]} (${FILLER_ADDRS[$i]}) — USDC on B: ${USDC_B_BEFORE[$i]} | WETH on A: ${WETH_A_BEFORE[$i]}"
done
...
log STEP "Step 3 — Creating cross-chain order (2000 USDC → 1 WETH, 2 of N slots will be filled)"

CURRENT_BLOCK_B=$(cast block-number --rpc-url "$RPC_B")
DEADLINE=$((CURRENT_BLOCK_B + 300))
T2_BUFFER=50

INPUT_AMOUNT="2000000000"           # 2000 USDC (6 decimals) → SlotLib: 4 slots
MIN_OUTPUT="1000000000000000000"   # 1 WETH → 0.25 WETH per slot
NONCE="2"                          # distinct from run_cc.sh's NONCE=1; bump if re-run
...
log INFO "T1 = Chain B block $DEADLINE | T2 = Chain B block $((DEADLINE - T2_BUFFER))"
...
DOMAIN_SEPARATOR=$(cast call "$ESCROW_SRC_FACTORY_B" "DOMAIN_SEPARATOR()(bytes32)" --rpc-url "$RPC_B")
...
# ── 5. Each filler calls EscrowSrcFactory.fillSlot() on Chain B ──────────────
...
# swapper's USDC into it via Permit2. msg.value is the filler's safety deposit.
log STEP "Step 5 — Chain B: each filler calls fillSlot() (deploys EscrowSrc clone + pulls USDC via Permit2)"

SAFETY_DEPOSIT="10000000000000000"  # 0.01 ETH per slot

declare -a ESCROW_ADDR_B
for i in 0 1; do
  slot=${SLOTS[$i]}
  name=${FILLER_NAMES[$i]}
  pk=${FILLER_PKS[$i]}
  ...
  run_cmd "$name: fillSlot(info, swapperSig, cosignerSig, slot=$slot, H_$slot, proof)" \
    cast send "$ESCROW_SRC_FACTORY_B" \
      "fillSlot((address,address,uint256,address,uint256,uint256,uint256,bytes32,uint8),bytes,bytes,uint8,bytes32,bytes32[])" \
      "($ACCOUNT0,$USDC,$INPUT_AMOUNT,$WETH,$MIN_OUTPUT,$DEADLINE,$NONCE,$MERKLE_ROOT,$NUM_SLOTS)" \
      "$SWAPPER_SIG" "$COSIGNER_SIG" "$slot" "${HASHLOCK[$i]}" "${PROOF_CAST[$i]}" \
      --value "$SAFETY_DEPOSIT" --private-key "$pk" --rpc-url "$RPC_B"

  ESCROW_ADDR_B[$i]=$(cast call "$ESCROW_SRC_FACTORY_B" "computeAddress(bytes32,uint8)(address)" "$ORDER_HASH" "$slot" --rpc-url "$RPC_B")
  log OK "$name — EscrowSrc clone deployed at ${ESCROW_ADDR_B[$i]} (slot $slot) on Chain B"
done

# ── 6. Both fillers fill their slot via EscrowDstFactory on Chain A ──────────
log STEP "Step 6 — Chain A: each filler deploys its own escrow clone (no approve needed)"

SLOT_AMOUNT=$(python3 -c "print($MIN_OUTPUT // $NUM_SLOTS)")   # WETH per slot

CURRENT_BLOCK_A=$(cast block-number --rpc-url "$RPC_A")
T2_EXPIRY=$((CURRENT_BLOCK_A + DEADLINE - CURRENT_BLOCK_B - T2_BUFFER))
log INFO "Slot amount: $SLOT_AMOUNT WETH | T2 expiry (Chain A block): $T2_EXPIRY"
...
for i in 0 1; do
  ...
  ESCROW_ADDR_A[$i]=$(cast call "$CHAIN_A_DST_FACTORY" \
    "computeAddress(bytes32,address)(address)" \
    "$H" "$addr" \
    --rpc-url "$RPC_A")
  log OK "$name — precomputed escrow address (slot $slot): ${ESCROW_ADDR_A[$i]}"

  # Step 6b: transfer WETH directly to the escrow address (no approve!)
  run_cmd "$name: WETH.transfer(escrowAddr, $SLOT_AMOUNT) — fund clone for slot $slot" \
    cast send "$WETH" "transfer(address,uint256)" \
      "${ESCROW_ADDR_A[$i]}" "$SLOT_AMOUNT" \
      --private-key "$pk" --rpc-url "$RPC_A"

  ESCROW_WETH=$(cast call "$WETH" "balanceOf(address)(uint256)" "${ESCROW_ADDR_A[$i]}" --rpc-url "$RPC_A")
  log OK "$name — escrow holds $ESCROW_WETH WETH (before deployment)"
  ...
  run_cmd "$name: EscrowDstFactory.deploy() — deploy + initialize clone for slot $slot" \
    cast send "$CHAIN_A_DST_FACTORY" \
      "deploy(bytes32,address,address,uint256,uint256)" \
      "$H" "$ACCOUNT0" "$WETH" "$SLOT_AMOUNT" "$T2_EXPIRY" \
      --private-key "$pk" --rpc-url "$RPC_A"

  log OK "$name — EscrowDst clone deployed at ${ESCROW_ADDR_A[$i]} (slot $slot) on Chain A"
done

# ── 7. Mine Chain A blocks for confirmation ───────────────────────────────────
log STEP "Step 7 — Mining Chain A blocks for confirmation"
for i in 1 2 3; do
  cast rpc anvil_mine 1 --rpc-url "$RPC_A" >> "$LOG_FILE" 2>&1
  log INFO "Mined block on Chain A — now at $(cast block-number --rpc-url $RPC_A)"
done

# ── 8. Wait for backend ChainA watcher to claim from BOTH escrows ─────────────
log STEP "Step 8 — Waiting for backend to claim from both EscrowDst clones"
log INFO "Watcher listens for EscrowCreated on Chain A, re-derives S_i, calls escrow.claim(S_i) for each"
...
if [ "$CLAIMED" != "true" ]; then
  ...
  log WARN  "Check backend logs for '[ChainA]' EscrowCreated events from $RPC_A"
  exit 1
fi

# ── 9. Read S_i from each escrow's Claimed event on Chain A ──────────────────
log STEP "Step 9 — Reading S_i from each escrow's Claimed event on Chain A"
log INFO "event Claimed(address indexed claimer, bytes32 secret) — data field = S_i (now public)"

declare -a SECRET
for i in 0 1; do
  slot=${SLOTS[$i]}
  escrow=${ESCROW_ADDR_A[$i]}

  SECRET[$i]=$(python3 << PYEOF
import urllib.request, json, subprocess

escrow = "$escrow"
from_block = hex($CURRENT_BLOCK_A)
...
PYEOF
  ...
  log OK "S_$slot (public on Chain A) = ${SECRET[$i]}"
done

# ── 10. Each filler withdraws from its EscrowSrc clone on Chain B ────────────
...
# keccak256(secret) == hashlock and pays out the USDC amount + safety deposit.
log STEP "Step 10 — Chain B: each filler withdraws USDC + safety deposit using its own secret"
...
for i in 0 1; do
  ...
  run_cmd "$name: EscrowSrc(${ESCROW_ADDR_B[$i]}).withdraw(S_$slot)" \
    cast send "${ESCROW_ADDR_B[$i]}" \
      "withdraw(bytes32)" \
      "${SECRET[$i]}" \
...
declare -a USDC_B_AFTER WETH_A_AFTER USDC_GAINED WETH_SPENT ESCROW_STATUS_B
for i in 0 1; do
  ...
  USDC_GAINED[$i]=$(python3 -c "print(${USDC_B_AFTER[$i]} - ${USDC_B_BEFORE[$i]})")
  WETH_SPENT[$i]=$(python3  -c "print(${WETH_A_BEFORE[$i]} - ${WETH_A_AFTER[$i]})")
  ESCROW_STATUS_B[$i]=$(cast call "${ESCROW_ADDR_B[$i]}" "status()(string)" --rpc-url "$RPC_B" | tr -d '"')
done
...
WETH_GAINED=$(python3 -c "print($WETH_SWAPPER_A_AFTER - $WETH_SWAPPER_A_BEFORE)")
USDC_SWAPPER_SPENT=$(python3 -c "print($USDC_SWAPPER_B_BEFORE - $USDC_SWAPPER_B_AFTER)")
SLOT_USDC=$(python3   -c "print($INPUT_AMOUNT // $NUM_SLOTS)")

log RAW ""
log RAW "  ╔══════════════════════════════════════════════════════════════════╗"
log RAW "  ║  Cross-Chain Swap Results (B→A) — 2 fillers, slots ${SLOTS[0]} & ${SLOTS[1]} of $NUM_SLOTS          ║"
log RAW "  ╠══════════════════════════════════════════════════════════════════╣"
for i in 0 1; do
...
for i in 0 1; do
  ...
  if python3 -c "exit(0 if ${USDC_B_AFTER[$i]} > ${USDC_B_BEFORE[$i]} else 1)"; then
    log OK "✔ $name gained ${USDC_GAINED[$i]} USDC on Chain B (expected ~$SLOT_USDC)"
  else
    log ERROR "✖ $name USDC on Chain B did not increase"; PASS=false
  fi

  if python3 -c "exit(0 if ${WETH_A_BEFORE[$i]} > ${WETH_A_AFTER[$i]} else 1)"; then
    log OK "✔ $name spent ${WETH_SPENT[$i]} WETH on Chain A (locked in escrow clone)"
  else
    log ERROR "✖ $name WETH on Chain A did not decrease"; PASS=false
  fi

  if [ "${ESCROW_STATUS_B[$i]}" = "withdrawn" ]; then
    log OK "✔ $name's EscrowSrc clone (slot ${SLOTS[$i]}) status = withdrawn (safety deposit returned)"
  else
    log ERROR "✖ $name's EscrowSrc clone (slot ${SLOTS[$i]}) status = ${ESCROW_STATUS_B[$i]} (expected withdrawn)"; PASS=false
  fi
done

if python3 -c "exit(0 if $WETH_SWAPPER_A_AFTER > $WETH_SWAPPER_A_BEFORE else 1)"; then
  log OK "✔ Swapper gained $WETH_GAINED WETH on Chain A (both escrow.claim() calls sent it)"
else
  log ERROR "✖ Swapper WETH on Chain A did not increase"; PASS=false
fi

if python3 -c "exit(0 if $USDC_SWAPPER_B_AFTER < $USDC_SWAPPER_B_BEFORE else 1)"; then
  log OK "✔ Swapper spent $USDC_SWAPPER_SPENT USDC on Chain B (pulled via Permit2 into both EscrowSrc clones)"
else
  log ERROR "✖ Swapper USDC on Chain B did not decrease"; PASS=false
fi
...
log RAW ""
log RAW "  EscrowSrc/EscrowDst clone flow confirmed (B→A) across two Anvil instances with 2 concurrent fillers:"
log RAW ""
log RAW "  Chain B (port 8546):"
for i in 0 1; do
  log RAW "    ${FILLER_NAMES[$i]} fillSlot()'d slot ${SLOTS[$i]} → EscrowSrc ${ESCROW_ADDR_B[$i]} → withdrew ${USDC_GAINED[$i]} USDC + safety deposit"
done
log RAW ""
log RAW "  Chain A (port 8545):"
for i in 0 1; do
  log RAW "    ${FILLER_NAMES[$i]} deployed EscrowDst clone at ${ESCROW_ADDR_A[$i]}"
done
log RAW "    backend claimed both clones → revealed S_${SLOTS[0]}, S_${SLOTS[1]} → swapper received $WETH_GAINED WETH total"
log RAW ""
