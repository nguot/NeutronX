...
# NOTE: bump FORK_BLOCK in scripts/fork_block.sh every so often — a block
# pinned too far in the past starts failing `eth_feeHistory` ("pruned history
# unavailable") on Alchemy's free tier, AND the hardcoded whale addresses
# (funding/, filler/*/src/funding/, scripts/crosschain/setup_cc.sh,
# scripts/race/common.sh) can drain over time (verified: the old 20500000
# pin's USDC whales had gone from ~543M/45K USDC down to ~$0/~45K by 2026-07 —
# re-check whale balances at the new block with
# `cast call <token> "balanceOf(address)(uint256)" <whale> --block <N>`
